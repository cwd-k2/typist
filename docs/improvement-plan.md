...
> **Implementation note:** `TypeChecker._check_assignments` を新設。`:Type(...)` で注釈された変数への再代入のみを検査 (annotated-only guard)。`PPI::Statement::Variable` 内の初期化はスキップし、`_env_for_node` によるスコープ aware な環境でチェック。
