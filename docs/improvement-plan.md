...
> **Status: All Phases Completed** (2026-03-01)
> Phase A-E の全タスクが実装完了。本文書は歴史的参照として維持する。
...
**✅ 完了** — `ArityMismatch` 診断、`_extract_args` deref chain 修正

> **Implementation note:** `TypeChecker._check_call_sites` にアリティ検査を追加。引数の過不足を `ArityMismatch` として報告。`_extract_args` の deref chain (`$arr->[0]` 等) を正しくグルーピングする修正を含む。
...
**✅ 完了** — 算術/比較/論理/結合/添字/三項演算子

> **Implementation note:** `Static::Infer` に二項演算パターン検出を実装。算術演算→Num、比較→Bool、文字列結合→Str、論理演算→左辺型、否定→Bool。添字アクセス (`$arr->[0]`, `$hash->{key}`) と三項演算子 (`$x ? $a : $b` → Union) も対応。
...
**✅ 完了** — annotated-only guard、スコープ aware env

> **Implementation note:** `TypeChecker._check_assignments` を新設。`:sig(...)` で注釈された変数への再代入のみを検査 (annotated-only guard)。`PPI::Statement::Variable` 内の初期化はスキップし、`_env_for_node` によるスコープ aware な環境でチェック。
...
- `t/static/03_typecheck.t`:
  - `$x :sig(Int) = 0; $x = "hello"` → TypeMismatch
  - `$x :sig(Int) = 0; $x = 42` → OK
  - `my` 文内は重複チェックしない
...
**✅ 完了** — Phase 1-2（is_method, register_method, `$self->method()` チェック）

> **Implementation note:** 設計指示書 Phase 1 (防御的修正 + メソッド抽出基盤) と Phase 2 (同一パッケージ内メソッド型チェック) を実装。Extractor に `is_method` フラグ、Registry に `register_method`/`lookup_method`、TypeChecker に `_check_method_calls` を追加。Phase 3-4 (クロスパッケージ、チェーン対応) は将来課題。詳細は `docs/design/method-type-checking.md` 参照。
...
**✅ 完了** — Static::Unify、instantiation パイプライン

> **Implementation note:** `Typist::Static::Unify` を新設し、型ベースの unification を実装。`TypeChecker._check_call_sites` でジェネリック関数のスキップを廃止し、引数型から型変数を束縛→具象化→サブタイプチェックのパイプラインを構築。有界量化 (bounded quantification) のチェックも含む。
...
**✅ 完了** (Phase B-2a のみ) — `defined()` ガードで `Maybe[T]` → `T`

> **Implementation note:** Phase B-2a を実装。`TypeChecker._narrow_env` で `if (defined $x)` パターンを検出し、ブロック内で `Union(T | Undef)` → `T` にナローイング。Phase B-2b (ref チェック) および B-2c (Newtype ガード) は将来課題。
...
**✅ 完了** — `_check_implicit_return_of_stmt`

> **Implementation note:** `TypeChecker._check_return_types` を拡張し、`PPI::Statement::Compound` (if/elsif/else) の各ブロック内 return 文を再帰的に走査。`_check_implicit_return_of_stmt` で分岐ごとの戻り値型を検査。
...
**✅ 完了** — Type::Data、datatype キーワード、コンストラクタ生成

> **Implementation note:** `Typist::Type::Data` を新設。`datatype` キーワードで ADT を定義し、各バリアントのコンストラクタを呼び出し元パッケージにインストール。Registry への登録、Extractor での抽出、サブタイピング (名前的等価) を実装。
...
**✅ 完了** — `F[T]` パース、KindChecker 拡張

> **Implementation note:** Parser に型変数適用 `F[T]` のパースを追加。KindChecker で `Param(Var(F), [Atom(T)])` のカインド検査を実装。型コンストラクタ変数を関数シグネチャで自由に使用可能に。
...
> **Implementation note:** `TypeClass::Def` を複数型変数に対応。instance resolution の多引数マッチングを実装。`typeclass Convertible => 'T, U'` 形式をサポート。
...
> **Implementation note:** `Attribute._wrap_sub` 内で bound 式のパース結果をクロージャ生成前にキャッシュ。ラッパー内では事前パース済みの bound を参照するように変更。
...
**✅ 完了** — wantarray dispatch

> **Implementation note:** `Attribute._wrap_sub` のラッパークロージャで `wantarray` による3分岐ディスパッチ (list/scalar/void) を実装。呼び出し元のコンテキストを正しく伝播。
...
> **Implementation note:** `Tie::Scalar::DESTROY` で Registry からエントリを削除する方式を採用。`Scalar::Util::weaken` による弱参照化と組み合わせ、スコープ外の変数エントリを自動クリーンアップ。
...
> **Implementation note:** `Typist.pm` の `_unwrap` で使用する正規表現を `qr//` で事前コンパイルし、モジュールレベル変数 `$NEWTYPE_RE` に格納。
...
**✅ 完了** — Static::Unify として実装

> **Implementation note:** `Typist::Static::Unify` を新設し、`Inference.pm` の `_unify` ロジックを型ベースに移植。`unify($formal, $actual)` で型変数束縛を返し、`instantiate_call($sig, @arg_types)` で呼び出しサイトの具象化を提供。B-1 (ジェネリック静的チェック) と同時に実装。
...
> **Implementation note:** `Typist::Prelude` を新設。IO 系 (`say`, `print`, `warn`, `die`)、文字列操作 (`length`, `uc`, `lc` 等)、数値操作 (`abs`, `int`, `sqrt`)、リスト操作 (`scalar`, `reverse`, `sort`) のビルトイン関数型定義をプレリュードとして提供。`Typist.pm` の import 時に自動ロード。
...
**✅ 完了** — Checker 行番号、`_to_diagnostics` フォールバック削減

> **Implementation note:** Checker の `_check_functions` と `_check_aliases` で `extracted` データから行番号を取得し `collect` に渡すように修正。`Analyzer._to_diagnostics` の正規表現フォールバックを最小限に削減し、大部分の診断が直接的な行番号を持つように改善。
...
Phase 2 (検出力 — 基盤の上に構築):        ✅ 完了
  A-2a: 算術演算の推論               (中規模)
  A-3: 変数再代入追跡                (中規模)
  A-4: メソッド型チェック Phase 1    (設計書あり)

Phase 3 (深化 — 推論基盤を前提):          ✅ 完了
  A-2b: 添字アクセスの推論           (中規模)
  A-2c: 三項演算子の推論             (小規模)
  E-1: Static::Unify 新設            (B-1 の前提)
  B-1: ジェネリック静的チェック      (大規模)
  A-4: メソッド型チェック Phase 2    (中規模)

Phase 4 (表現力 — 独立して着手可能):      ✅ 完了
  E-2: ビルトインプレリュード        (中規模、効果テスト容易)
  B-3: 分岐戻り値解析               (中規模)
  D-3: Registry リーク修正          (小規模)
  E-3: ソースマップ精度             (中規模)

Phase 5 (高度な拡張 — 長期目標):          ✅ 完了
  B-2: Type Narrowing (B-2a)         (大規模)
  C-1: ADT / Tagged Union            (大規模)
  C-3: HKT 完全化                   (中規模)
  C-4: 多引数型クラス               (中規模)
  C-2: エフェクトハンドラ (C-2a+b)   (最大規模)
```
