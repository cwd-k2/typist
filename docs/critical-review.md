...
**対応**: GradualHint 診断（severity 5, opt-in）を関数境界で発行。
- `_contains_any` でスキップされた箇所に `GradualHint` kind のエラーを発行
- TypeChecker: return value / implicit return の Any スキップ時
- CallChecker: 引数の Any スキップ時
- LSP では Hint (severity 4) として表示、CLI では `--verbose` 時のみ表示
- `gradual_hints => 1` オプションで有効化（LSP はデフォルト有効）

**残課題**: 完全な blame tracking（どのアノテーション欠如が原因か）は型理論の研究課題
...
| 指摘 | 節 | 方針 |
|------|-----|------|
| Gradual Typing の完全な Blame tracking | §6.5 | GradualHint 診断で部分対応済み。完全な blame 追跡は型理論の研究課題 |
