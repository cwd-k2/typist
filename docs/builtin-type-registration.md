...
- `perform` は廃止済み — effect operations は直接 qualified call（`Effect::op(@args)`）で呼び出す
...
- `perform` は廃止済み — effect operations は `Effect::op(@args)` で呼び出す
...
- `unwrap` の CORE 登録確認（`Any` でグラデュアルスキップ）
- 宣言関数の `Void` 戻り値確認
